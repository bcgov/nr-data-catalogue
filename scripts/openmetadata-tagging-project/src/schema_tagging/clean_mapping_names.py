...
#     """
#     Clean name by:
#     1. Removing _REPLICATION if present
#     2. Removing dash and everything after it
...
#     - 'FTA_REPLICATION - odstst' -> 'FTA'
#     """
#     # First remove _REPLICATION if present
#     name = name.replace('_REPLICATION', '')
    
#     # Then remove dash and everything after it
#     name = name.split(' - ')[0]
...
#     Examples:
#     - 'GEOBC Test Database' -> 'GEOBC'
#     - 'Operational Data Store - DEV' -> 'ODS_DEV'
#     - 'TEST Operational Data Store' -> 'ODS_TEST'
#     """
...
#         if should_include_entry(app_name):
#             original_name = app_name
#             cleaned_name = clean_name(app_name)
#             service = details['service']
            
#             # Create unique key using cleaned name and service
#             unique_key = create_unique_key(cleaned_name, service)
...
#             if original_name != unique_key:
#                 cleaned_entries.append((original_name, unique_key))
#         else:
#             excluded_entries.append(app_name)
...
#     if cleaned_entries:
#         logging.info("\nCleaned entries:")
#         for original, cleaned in cleaned_entries:
#             logging.info(f"- {original} -> {cleaned}")
...
# def clean_name(name: str) -> str:
#     """Remove everything after and including the dash from a name"""
#     return name.split(' - ')[0] if ' - ' in name else name

# def clean_mapping_names(mapping_file: str):
#     """Clean application and tag names in the mapping file, excluding specific entries"""
#     # Read the original mapping
#     with open(mapping_file, 'r') as f:
#         mapping = json.load(f)
...
#     for app_name, details in mapping.items():
#         if should_include_entry(app_name):
#             cleaned_app_name = clean_name(app_name)
#             cleaned_details = details.copy()
#             cleaned_details['tag_name'] = clean_name(details['tag_name'])
#             cleaned_mapping[cleaned_app_name] = cleaned_details
#         else:
#             excluded_entries.append(app_name)
...
#     print(f"Original mapping file has been cleaned and updated: {mapping_file}")
    
#     # Print summary of changes
#     print(f"\nTotal entries processed: {len(mapping)}")
#     print(f"Entries in cleaned mapping: {len(cleaned_mapping)}")
#     print(f"Entries removed: {len(excluded_entries)}")
#     if excluded_entries:
#         print("\nRemoved entries:")
#         for entry in excluded_entries:
#             print(f"- {entry}")
...
#     sample_count = min(3, len(cleaned_mapping))
#     cleaned_items = list(cleaned_mapping.items())[:sample_count]
    
#     for new_name, new_details in cleaned_items:
#         original_name = next((name for name in mapping.keys() 
#                             if clean_name(name) == new_name), None)
#         if original_name:
#             print(f"\nOriginal: {original_name} -> {mapping[original_name]['tag_name']}")
#             print(f"Cleaned:  {new_name} -> {new_details['tag_name']}")
...
#     # Input file path (will be overwritten)
#     mapping_file = project_root / 'data' / 'application_mapping.json'
    
#     clean_mapping_names(str(mapping_file))